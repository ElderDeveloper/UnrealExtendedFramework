...
class QuestEdge:
    ...
    def __str__(self):
        return "QuestEdge(target_node_index = {}, text = {})".format(self.target_node_index, self.text)

    def __repr__(self):
        return str(self)


class QuestInnerEdge:
...
class QuestObjectiveSequenceNode:
...
def is_path_json_human_text(file_path):
    ...
    path = Path(file_path)
    if "".join(path.suffixes) != ".quest_human.json":
        return False
...
JSON_KEY_OBJECTIVE_NODES = "ObjectiveNodes"
JSON_KEY_OBJECTIVE_SEQUENCE_NODES = "ObjectiveSequenceNodes"
    ...
    if JSON_KEY_NODE_EDGES in node_json:
        for node_edge in node_json[JSON_KEY_NODE_EDGES]:
            if JSON_KEY_EDGE_TARGET_NODE_INDEX in node_edge and JSON_KEY_TEXT in node_edge:
                edge = QuestEdge(node_edge[JSON_KEY_EDGE_TARGET_NODE_INDEX], node_edge[JSON_KEY_TEXT])
                edges.append(edge)
    ...
    if JSON_KEY_NODE_SEQUENCE in node_json:
        for node_inner_edge in node_json[JSON_KEY_NODE_SEQUENCE]:
            if JSON_KEY_TEXT in node_inner_edge and JSON_KEY_INNER_EDGE_TEXT in node_inner_edge:
                inner_edge = QuestInnerEdge(node_inner_edge[JSON_KEY_TEXT], node_inner_edge[JSON_KEY_INNER_EDGE_TEXT])
...
def spellcheck_json_human_text(file_path):
    ...
    # Objective nodes
    objective_nodes = []
    objective_sequence_node = []
    if JSON_KEY_OBJECTIVE_NODES in file_json:
        for node_json in file_json[JSON_KEY_OBJECTIVE_NODES]:

            # Read node
            if JSON_KEY_NODE_INDEX in node_json and JSON_KEY_TEXT in node_json:
                node = QuestNode(node_json[JSON_KEY_NODE_INDEX], node_json[JSON_KEY_TEXT])
                node.edges = get_edges_from_node_json(node_json)
                objective_nodes.append(node)


    # Objective sequence nodes
    if JSON_KEY_OBJECTIVE_SEQUENCE_NODES in file_json:
        for node_json in file_json[JSON_KEY_OBJECTIVE_SEQUENCE_NODES]:
            # Read node
            if JSON_KEY_NODE_INDEX in node_json and JSON_KEY_NODE_SEQUENCE in node_json:
                node = QuestObjectiveSequenceNode(node_json[JSON_KEY_NODE_INDEX])
                node.edges = get_edges_from_node_json(node_json)
                node.inner_edges = get_inner_edges_from_node_json(node_json)
                objective_sequence_node.append(node)
    ...
    for node_sequence in objective_sequence_node:
        ...
        if warning_edges or warnings_inner_edges:
            print_yellow("Objective Sequence Node index = {}".format(node.node_index))
...
if __name__ == "__main__":
    ...
    parser.add_argument('directory', nargs="?", type=str, help='Directory containing all the json human text files', default="QuestsJsonHumanText/")
