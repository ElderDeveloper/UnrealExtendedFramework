...
QUEST_JSON_HUMAN_EXTENSION = ".quest_human.json"
ROOT_NODE_INDEX = -1
...
class TwineNodeTag:
    ...
    NODE_OBJECTIVE_SEQUENCE = "node-objective-sequence"
    NODE_SELECTOR_FIRST = "node-selector-first"
    NODE_SELECTOR_RANDOM = "node-selector-random"

    @classmethod
    def get_all_tags(cls):
        return set([cls.NODE_START, cls.NODE_END, cls.NODE_VIRTUAL_PARENT, cls.NODE_OBJECTIVE, cls.NODE_OBJECTIVE_SEQUENCE, cls.NODE_SELECTOR_FIRST, cls.NODE_SELECTOR_RANDOM])
...
class TwineHelper:
    ...
    @classmethod
    def parse_twine_node_name(cls, raw_name, context_multiple_matches, context_invalid_index, context_invalid_label):
        # Get node index and its human-readable label.
        matches_name = re.finditer(cls.REGEX_NAME, raw_name, re.MULTILINE | re.UNICODE)
        node_index, label = None, None
        for index, match in enumerate(matches_name):
            ...
            if group_index is not None:
                ...
                print_yellow("{}, could not get node index from <node index>. <Label>".format(context_invalid_index))

            group_label = match.group(2)
            if group_index is not None:
                label = group_label.strip()
            else:
                print_yellow("{}, could not get label from <node index>. <Label>".format(context_invalid_label))
...
class TwineEdgeData:
    ...
    def parse(self):
        ...
        self.target_node_index, ignored_label = TwineHelper.parse_twine_node_name(parts[1], context_parse_name, context_parse_name, context_parse_name)
...
class TwineInnerEdgeData:
    ...
    def parse(self):
        ...
        for index, match in enumerate(matches_text):
            if index > 0:
                print_yellow("Node objective sequence Index = {} got multiple matches for Text".format(self.owner_node_index))
                break

            group = match.group(1)
            if group is None:
                print_yellow("Node objective sequence Index = {} could not get group 1 that matches ``Text:`` //<text>//".format(self.owner_node_index))
                continue
        ...
        for index, match in enumerate(matches_edge_text):
            if index > 0:
                print_yellow("Node objective sequence Index = {} got multiple matches for edge text".format(self.owner_node_index))
                break

            group = match.group(1)
            if group is None:
                print_yellow("Node objective sequence Index = {} could not get group 1 that matches ``EdgeText:`` //<edge_text>//".format(self.owner_node_index))
                continue
...
class TwineNodeData:
    ...
    # only for objective sequence nodese
    def _parse_inner_edges(self):
        if not self.is_node_objective_sequence() or not self.raw_data:
            return

        raw_text_data = self.__get_raw_data_until_edges().strip()
        inner_edges_parts = raw_text_data.split("---")
        if not inner_edges_parts:
            print_yellow("Node Index = {} which is a objective sequence node does not have inner edges".format(self.node_index))
            return
    ...
    def parse(self):
        ...
        # Get node index and label.
        context_parse_name = "Node Name = {}".format(self.raw_name)
        self.node_index, self.label = TwineHelper.parse_twine_node_name(self.raw_name, context_parse_name, context_parse_name, context_parse_name)
    ...
    def can_have_text(self):
        return self.is_node_objective() or self.is_node_virtual_parent()

    def can_have_text_on_edges(self):
        return self.is_node_start() or self.is_node_objective() or self.is_node_objective_sequence()
    ...
    def is_node_objective(self):
        return TwineNodeTag.NODE_OBJECTIVE in self.tags

    def is_node_virtual_parent(self):
        return TwineNodeTag.NODE_VIRTUAL_PARENT in self.tags

    def is_node_objective_sequence(self):
        return TwineNodeTag.NODE_OBJECTIVE_SEQUENCE in self.tags
    ...
    def to_dict(self):
        ...
        if self.is_node_objective_sequence():
            return {
    ...
    def __str__(self):
        return "TwineNodeData(node_index = {}, label = {}, tags = {}, text = `{}`, edges = {})".format(self.node_index, self.label, self.tags, self.text, self.edges)
...
class TwineDocumentData:
    def __init__(self):
        ...
        self.quest_guid = None
        self.nodes = []

    def _parse_quest_guid(self):
        # Convert to default Unreal uuid
        temp_uuid = uuid.UUID(self.raw_guid)
        self.quest_guid = temp_uuid.hex.upper()

    def parse(self):
        self._parse_quest_guid()

    def to_dict(self):
        if self.quest_name is None or self.quest_guid is None or not self.nodes:
            return {}

        objective_nodes = []
        objective_sequence_nodes = []
        for node in self.nodes:
            if node.is_node_objective_sequence():
                objective_sequence_nodes.append(node.to_dict())
            elif node.is_node_objective() or node.is_node_virtual_parent() or node.is_node_start():
                objective_nodes.append(node.to_dict())
            else:
                # Ignore
                pass
        ...
        return {
            ...
            "QuestGUID": self.quest_guid,
            "ObjectiveNodes": objective_nodes,
            "ObjectiveSequenceNodes": objective_sequence_nodes
        }

    def __str__(self):
        return "TwineDocumentData(quest_name = {}, quest_guid = {}, nodes =\n{})".format(self.quest_name, self.quest_guid, "\n".join(str(n) for n in self.nodes))
        ...
        if tag == self.HTML_TAG_STORYDATA:
            ...
            for attr in attrs:
                ...
                if attr_name == self.HTML_ATTR_NAME:
                    self.document.quest_name = attr_value.strip()
...
def export_twine_file_quest_text_json(src_file_path, src_twine_dir_from, dst_json_dir):
    ...
    dst_file_path = os.path.join(dst_dirname, src_file) + QUEST_JSON_HUMAN_EXTENSION
    ...
    for path, subdirs, files in os.walk(src_twine_dir):
        for name in files:
            ...
            if is_path_twine_file(full_filename):
                export_twine_file_quest_text_json(full_filename, src_twine_dir_from, dst_json_dir)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('src_twine_dir', nargs='?', type=str, help='Source directory from where we get all the .html twine files', default="QuestsTwine/")
    parser.add_argument('dst_json_dir', nargs='?', type=str, help='Destination directory where we store all the .quest_human.json files',  default="QuestsJsonHumanText/")
