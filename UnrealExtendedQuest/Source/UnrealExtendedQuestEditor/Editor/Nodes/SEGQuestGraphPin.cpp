...
#define LOCTEXT_NAMESPACE "SEGQuestGraphPin"

namespace
{
	bool IsObjectiveOutcomePin(const UEdGraphPin* Pin)
	{
		return Pin &&
			(Pin->PinType.PinCategory == UEGQuestEdGraphSchema::PIN_CATEGORY_Success ||
			 Pin->PinType.PinCategory == UEGQuestEdGraphSchema::PIN_CATEGORY_Fail);
	}
}

/////////////////////////////////////////////////////
// SEGQuestGraphPin
void SEGQuestGraphPin::Construct(const FArguments& InArgs, UEdGraphPin* InPin)
{
	check(InPin);
	...
	// An objective outcome pin is a small circle on its row; the root/custom output pin fills the
	// whole node body so wires can be dragged from anywhere on it.
	const FMargin PinPadding = IsObjectiveOutcomePin(InPin) ? FMargin(7.f) : FMargin(10.f);
...
FReply SEGQuestGraphPin::OnPinMouseDown(const FGeometry& SenderGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SEGQuestGraphPin::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Unhandled();
}

FReply SEGQuestGraphPin::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return Super::OnMouseMove(MyGeometry, MouseEvent);
}

void SEGQuestGraphPin::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (!bLocalIsHovered && GraphPinObj && !GraphPinObj->IsPendingKill() && GraphPinObj->GetOuter())
	{
		...
		{
			// Highlight everything this pin's wires lead to
			for (UEdGraphPin* LinkedPin : GraphPinObj->LinkedTo)
			{
				HoverPinSet.Add(LinkedPin);
				OwnerPanelPtr->AddPinToHoverSet(LinkedPin);
			}
		}
	}
...
void SEGQuestGraphPin::OnMouseLeave(const FPointerEvent& MouseEvent)
{
...
//
void SEGQuestGraphPin::OnDragEnter(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
...
void SEGQuestGraphPin::OnDragLeave(const FDragDropEvent& DragDropEvent)
{
	// NOTE: Super default implementation will not work properly with nodes that are not GetIsConnectable
#if NY_ENGINE_VERSION < 500
...
FReply SEGQuestGraphPin::OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	return Super::OnDragOver(MyGeometry, DragDropEvent);
}

FReply SEGQuestGraphPin::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	// NOTE: Super default implementation will not work properly with nodes that are not GetIsConnectable
#if NY_ENGINE_VERSION < 500
	...
	if (Operation->IsOfType<FGraphEditorDragDropAction>())
	{
		...
		FNYVector2f NodeAddPosition = FNYVector2f::ZeroVector;
		...
		if (OwnerNode.IsValid())
		{
...
#else
			NodeAddPosition	= OwnerNode->GetPosition() + FNYVector2f{MyGeometry.Position};
#endif
			...
			{
				const FNYVector2f PinOffsetPosition = FNYVector2f{MyGeometry.AbsolutePosition} - FNYVector2f{NodeWidget->GetTickSpaceGeometry().AbsolutePosition};
...
FSlateColor SEGQuestGraphPin::GetPinColor() const
{
	...
	// Outcome pins wear their outcome: green leads on success, red leads on fail.
	if (GraphPinObj->PinType.PinCategory == UEGQuestEdGraphSchema::PIN_CATEGORY_Success)
	{
		return IsHovered() ? Settings->WireSuccessArrowColor * 1.6f : Settings->WireSuccessArrowColor;
	}
	if (GraphPinObj->PinType.PinCategory == UEGQuestEdGraphSchema::PIN_CATEGORY_Fail)
	{
		return IsHovered() ? Settings->WireFailArrowColor * 1.6f : Settings->WireFailArrowColor;
	}

	if (UEGQuestGraphNode* GraphNode = Cast<UEGQuestGraphNode>(GraphPinObj->GetOwningNode()))
	{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Begin own functions
FReply SEGQuestGraphPin::OnAltAndLeftMouseButtonDown(const FGeometry& SenderGeometry, const FPointerEvent& MouseEvent)
{
	...
	const UEGQuestEdGraphSchema* Schema = CastChecked<UEGQuestEdGraphSchema>(GraphPinObj->GetSchema());
	const FNYVector2f& MouseLocation = MouseEvent.GetScreenSpacePosition();

	// Is the click inside the node?
	if (FEGQuestEditorUtilities::IsPointInsideGeometry(MouseLocation, ThisOwnerNodeWidget->GetCachedGeometry()))
	{
		// break all links
...
FReply SEGQuestGraphPin::OnCtrlAndLeftMouseButtonDown(const FGeometry& SenderGeometry, const FPointerEvent& MouseEvent)
{
	...
	// Start drag operation, killing the current connection
	const UEGQuestEdGraphSchema* Schema = CastChecked<UEGQuestEdGraphSchema>(GraphPinObj->GetSchema());
	Schema->BreakLinkTo(GraphPinObj, ToPin, true);
...
FText SEGQuestGraphPin::GetTooltipText() const
{
...
UEdGraphPin* SEGQuestGraphPin::GetBestLinkedToPinFromSplineMousePosition(const FNYVector2f& MousePosition) const
{
	/*
	...
	*/
	...
	// Iterate over all linked destinations, find the best one
	int32 BestLinkIndex = 0;

	// Dot product is always positive here, because we do not care about direction
	float BestDotProduct = 0.0f;
	const int32 LinkedNum = GraphPinObj->LinkedTo.Num();
	for (int32 LinkIndex = 0; LinkIndex < LinkedNum; LinkIndex++)
	{
		const UEGQuestGraphNode_Base* ChildNode = Cast<UEGQuestGraphNode_Base>(GraphPinObj->LinkedTo[LinkIndex]->GetOwningNodeUnchecked());
		if (!ChildNode)
		{
			continue;
		}
		TSharedPtr<SGraphNode> ChildNodeWidget = ChildNode->GetNodeWidget();
		if (!ChildNodeWidget.IsValid())
		{
			continue;
		}
	...
	return GraphPinObj->LinkedTo[BestLinkIndex];
}
// End own functions
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
