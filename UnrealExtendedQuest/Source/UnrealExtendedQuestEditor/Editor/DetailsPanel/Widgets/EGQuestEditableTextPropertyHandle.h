...
class UNREALEXTENDEDQUESTEDITOR_API FEGQuestEditableTextPropertyHandle : public IEditableTextProperty
{
public:
	FEGQuestEditableTextPropertyHandle(const TSharedRef<IPropertyHandle>& InPropertyHandle, const TSharedPtr<IPropertyUtilities>& InPropertyUtilities)
	...
	) const override
	{
		if (PropertyHandle->IsValidHandle())
		{
			...
			FEGQuestLocalizationHelper::StaticStableTextId(PropertyPackages[InIndex], InEditAction, InTextSource, InProposedNamespace, InProposedKey, OutStableNamespace, OutStableKey);
