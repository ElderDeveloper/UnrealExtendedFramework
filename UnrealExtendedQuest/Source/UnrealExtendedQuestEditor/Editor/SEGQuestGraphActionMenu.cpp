...
SEGQuestGraphActionMenu::~SEGQuestGraphActionMenu()
{
	OnClosedCallback.ExecuteIfBound();
	OnCloseReasonCallback.ExecuteIfBound(bActionExecuted, DraggedFromPins.Num() > 0);
}

void SEGQuestGraphActionMenu::Construct(const FArguments& InArgs)
{
...
void SEGQuestGraphActionMenu::CollectAllActions(FGraphActionListBuilderBase& OutAllActions)
{
...
void SEGQuestGraphActionMenu::OnActionSelected(const TArray<TSharedPtr<FEdGraphSchemaAction>>& SelectedAction, ESelectInfo::Type InSelectionType)
{
	if (!IsValid(Graph))
	{
		return;
