...
#define LOCTEXT_NAMESPACE "SEGQuestFindInQuests"

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// SEGQuestFindInQuests
void SEGQuestFindInQuests::Construct(const FArguments& InArgs, const TSharedPtr<FEGQuestEditor>& InQuestEditor)
{
	...
	// Only search in the current Quest
	bIsInFindWithinQuestMode = QuestEditorPtr.IsValid();

	constexpr bool bHostFindInQuestsInGlobalTab = true;
	ChildSlot
	[
		...
		[
			...
			[
				...
				.HintText(LOCTEXT("QuestSearchHint", "Enter function or event name to find references..."))
			...
			[
				...
				.ContentPadding(0)
				.ToolTipText(LOCTEXT("Filters_Tooltip", "Filter options for the Quest Search."))
			...
			[
				...
				// Show button if we are in a QuestEditor
				.Visibility(QuestEditorPtr.IsValid() && bHostFindInQuestsInGlobalTab ? EVisibility::Visible : EVisibility::Collapsed)
				.ToolTipText(LOCTEXT("OpenInGlobalFindResultsButtonTooltip", "Find in all Quests"))
				[
					SNew(STextBlock)
			...
			[
				...
				.Visibility(InArgs._bHideSearchBar || bHostFindInQuestsInGlobalTab ? EVisibility::Collapsed : EVisibility::Visible)
				[
					SNew(STextBlock)
		...
		[
			...
			[
				SAssignNew(TreeView, STreeView<TSharedPtr<FEGQuestSearchResult>>)
				// .ItemHeight(24)
...
SEGQuestFindInQuests::~SEGQuestFindInQuests()
{
	// TODO clear all cache for this
}

void SEGQuestFindInQuests::FocusForUse(bool bSetFindWithinQuest, const FEGQuestSearchFilter& SearchFilter, bool bSelectFirstResult)
{
	...
	// Set the filter mode
	bIsInFindWithinQuestMode = bSetFindWithinQuest;
	...
	if (!SearchFilter.SearchString.IsEmpty())
	{
		...
		MakeSearchQuery(SearchFilter, bIsInFindWithinQuestMode);
...
void SEGQuestFindInQuests::MakeSearchQuery(const FEGQuestSearchFilter& SearchFilter, bool bInIsFindWithinQuest)
{
	SearchTextBoxWidget->SetText(FText::FromString(SearchFilter.SearchString));
	...
	RootSearchResult = MakeShared<FEGQuestSearchResult_RootNode>();

	// TODO use threads extend FRunnable
	if (bInIsFindWithinQuest)
	{
		// Local
		if (QuestEditorPtr.IsValid())
		{
			FEGQuestSearchManager::Get()
					->QuerySingleQuest(SearchFilter, QuestEditorPtr.Pin()->GetQuestBeingEdited(), RootSearchResult);

			// Do now show the Quest in the search results.
			const TArray<TSharedPtr<FEGQuestSearchResult>>& Children = RootSearchResult->GetChildren();
			if (Children.Num() == 1 && Children[0].IsValid())
			{
				// Make the root be the first result (aka de quest).
				// NOTE: we must keep a reference here otherwise it crashes inside the parent reset
				TSharedPtr<FEGQuestSearchResult> TempChild = Children[0];
	...
	{
		// Global
		FEGQuestSearchManager::Get()->QueryAllQuests(SearchFilter, RootSearchResult);
	}
	...
	{
		// Some Items found
		ItemsFound.Add(MakeShared<FEGQuestSearchResult>(LOCTEXT("QuestSearchNoResults", "No Results found"), RootSearchResult));
...
FName SEGQuestFindInQuests::GetHostTabId() const
{
...
void SEGQuestFindInQuests::CloseHostTab()
{
	TSharedPtr<SDockTab> HostTabPtr = HostTab.Pin();
	if (HostTabPtr.IsValid())
	{
...
void SEGQuestFindInQuests::HandleHostTabClosed(TSharedRef<SDockTab> DockTab)
{
	FEGQuestSearchManager::Get()->CloseGlobalFindResults(SharedThis(this));
}

void SEGQuestFindInQuests::HandleSearchTextChanged(const FText& Text)
{
	CurrentFilter.SearchString = Text.ToString();
}

void SEGQuestFindInQuests::HandleSearchTextCommitted(const FText& Text, ETextCommit::Type CommitType)
{
	if (CommitType == ETextCommit::OnEnter)
	{
		CurrentFilter.SearchString = Text.ToString();
		MakeSearchQuery(CurrentFilter, bIsInFindWithinQuestMode);
	}
}

FReply SEGQuestFindInQuests::HandleOpenGlobalFindResults()
{
	TSharedPtr<SEGQuestFindInQuests> GlobalFindResults = FEGQuestSearchManager::Get()->GetGlobalFindResults();
...
void SEGQuestFindInQuests::HandleGetChildren(TSharedPtr<FEGQuestSearchResult> InItem, TArray<TSharedPtr<FEGQuestSearchResult>>& OutChildren)
{
	OutChildren += InItem->GetChildren();
}

void SEGQuestFindInQuests::HandleTreeSelectionDoubleClicked(TSharedPtr<FEGQuestSearchResult> Item)
{
	if (Item.IsValid())
	{
		Item->OnClick();
...
TSharedRef<ITableRow> SEGQuestFindInQuests::HandleGenerateRow(TSharedPtr<FEGQuestSearchResult> InItem, const TSharedRef<STableViewBase>& OwnerTable)
{
	// We have categories if we are searching in multiple Quests
	// OR the grandparent of this item is not valid (aka root node)
	const bool bIsCategoryWidget = !bIsInFindWithinQuestMode &&
		(!InItem->GetParent().IsValid() || (InItem->GetParent().IsValid() && InItem->GetParent().Pin()->IsRoot()));

	// Category entry
	if (bIsCategoryWidget)
	{
		return SNew(STableRow<TSharedPtr<FEGQuestSearchResult>>, OwnerTable)
			[
				SNew(SBorder)
				...
				[
					...
					[
						SNew(STextBlock)
						.Text(InItem.Get(), &FEGQuestSearchResult::GetDisplayText)
						.ToolTipText(LOCTEXT("QuestCatSearchToolTip", "Quest"))
					]
				]
			];
	}
	...
	FText Tooltip = FText::Format(LOCTEXT("QuestResultSearchToolTip", "{Category} : {DisplayTitle}"), Args);
...
TSharedPtr<SWidget> SEGQuestFindInQuests::HandleContextMenuOpening()
{
...
TSharedRef<SWidget> SEGQuestFindInQuests::FillFilterEntries()
{
	...
	MenuBuilder.AddMenuEntry(
		LOCTEXT("IncludeQuestGUID", "Include Quest GUIDs"),
		LOCTEXT("IncludeQuestGUID_ToolTip", "Include Quest GUIDs in search in the search result"),
		FSlateIcon(),
		FUIAction(
			FExecuteAction::CreateLambda([this]()
			{
				CurrentFilter.bIncludeQuestGUID = !CurrentFilter.bIncludeQuestGUID;
				MakeSearchQuery(CurrentFilter, bIsInFindWithinQuestMode);
			}),
			FCanExecuteAction(),
			FIsActionChecked::CreateLambda([this]() -> bool
			{
				return CurrentFilter.bIncludeQuestGUID;
			})
		),
		NAME_None,
		EUserInterfaceActionType::ToggleButton
	...
	MenuBuilder.AddMenuEntry(
		...
		LOCTEXT("IncludeCustomObjectNames_ToolTip", "Include the Custom Text Argument/Objective/Event/Node Data object names"),
		FSlateIcon(),
		FUIAction(
			FExecuteAction::CreateLambda([this]()
			{
