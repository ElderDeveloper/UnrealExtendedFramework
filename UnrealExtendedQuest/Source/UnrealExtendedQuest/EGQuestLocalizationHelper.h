 ...
 */
class UNREALEXTENDEDQUEST_API FEGQuestLocalizationHelper
{
	typedef FEGQuestLocalizationHelper Self;
public:
	static void UpdateTextFromRemapping(const UEGQuestPluginSettings& Settings, FText& OutText);

#if WITH_EDITOR
	// Updates the text namespace to match the settings options
	// NOTE: only works in editor mode
	static void UpdateTextNamespaceAndKey(const UObject* Object, const UEGQuestPluginSettings& Settings, FText& Text);
#else
	// NO OP
	static void UpdateTextNamespaceAndKey(const UObject* Object, const UEGQuestPluginSettings& Settings, FText& Text) {}
#endif
	...
	static bool WillTextNamespaceBeUpdated(const FText& Text, const UEGQuestPluginSettings& Settings);
