...
DEFINE_LOG_CATEGORY_STATIC(LogEGQuestPluginReflectionHelper, All, All)

class UNREALEXTENDEDQUEST_API FNYReflectionHelper
{
public:
	...
	static VariableType GetVariable(const UObject* Object, FName VariableName)
	{
		...
		{
			UE_LOG(
				LogEGQuestPluginReflectionHelper,
				Error,
	...
	// Gathers property names in ObjectClass, excluding properties declared by blacklisted base classes.
	...
	{
		if (!IsValid(ObjectClass) || !PropertyClass)
		{
			return;
		}
		...
		auto* Property = ObjectClass->PropertyLink;
