...
#include "UnrealExtendedQuest/NYReflectionHelper.h"
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void FEGQuestJsonParser::InitializeParser(const FString& FilePath)
{
	...
	{
		UE_LOG(LogQuestJsonParser, Error, TEXT("Failed to load config file %s"), *FilePath);
		bIsValidFile = false;
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void FEGQuestJsonParser::InitializeParserFromString(const FString& Text)
{
	JsonString = Text;
	bIsValidFile = true;
	FileName = "";
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void FEGQuestJsonParser::ReadAllProperty( const UStruct* ReferenceClass, void* TargetObject, UObject* InDefaultObjectOuter)
{
	if (!IsValidFile())
	{
		return;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool FEGQuestJsonParser::ConvertScalarJsonValueToProperty(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* ContainerPtr, void* ValuePtr)
{
	check(Property);
	if (bLogVerbose)
	{
		UE_LOG(LogQuestJsonParser, Verbose, TEXT("ConvertScalarJsonValueToProperty, Property = `%s`"), *Property->GetPathName());
	}
	if (ValuePtr == nullptr)
	{
		// Nothing else to do
	...
	{
		// ValuePtr is always the property's value storage, including array/map elements.
		// Use FObjectProperty accessors so raw pointers and TObjectPtr fields are both handled correctly.
		if (ValuePtr == nullptr)
		{
			UE_LOG(
				LogQuestJsonParser,
				Error,
				TEXT("PropertyName = `%s` is a FObjectProperty but has null value storage"),
				*Property->GetNameCPP()
			);
			return false;
		}
		...
		if (JsonValue->Type == EJson::String)
		{
			...
			{
				ObjectProperty->SetObjectPropertyValue(ValuePtr, StaticLoadObject(UObject::StaticClass(), DefaultObjectOuter, *Path));
			}
			return true;
		}
		...
		UObject* NewObjectValue = CreateNewUObject(ChildClass, DefaultObjectOuter);
		ObjectProperty->SetObjectPropertyValue(ValuePtr, NewObjectValue);

		// Something is wrong
		if (NewObjectValue == nullptr || !NewObjectValue->IsValidLowLevelFast())
		{
			UE_LOG(
				LogQuestJsonParser,
				Error,
		...
		if (!JsonObjectToUStruct(JsonObject.ToSharedRef(), ObjectClass, NewObjectValue))
		{
			UE_LOG(
				LogQuestJsonParser,
				Error,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool FEGQuestJsonParser::JsonValueToProperty(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* ContainerPtr, void* ValuePtr)
{
	check(Property);
	if (bLogVerbose)
	{
		UE_LOG(LogQuestJsonParser, Verbose, TEXT("JsonValueToProperty, Property = `%s`"), *Property->GetPathName());
	}
	if (!JsonValue.IsValid())
	{
		UE_LOG(LogQuestJsonParser, Error, TEXT("JsonValueToProperty - Invalid value JSON key"));
		return false;
	}
	...
	if (!bJsonArray)
	{
		...
		{
			UE_LOG(LogQuestJsonParser, Warning, TEXT("[Property->ArrayDim != 1] Ignoring excess properties when deserializing %s"), *Property->GetNameCPP());
		}
	...
	{
		UE_LOG(LogQuestJsonParser, Warning, TEXT("[Property->ArrayDim < ArrayValue.Num()] Ignoring excess properties when deserializing %s"), *Property->GetNameCPP());
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool FEGQuestJsonParser::JsonAttributesToUStruct(const TMap<FString, TSharedPtr<FJsonValue>>& JsonAttributes,
											const UStruct* StructDefinition, void* ContainerPtr)
{
	...
	if (bLogVerbose)
	{
		UE_LOG(LogQuestJsonParser, Verbose, TEXT("JsonAttributesToUStruct, StructDefinition = `%s`"), *StructDefinition->GetPathName());
	}
	...
	if (StructDefinition == FJsonObjectWrapper::StaticStruct())
	{
		...
		// UE 5.8: FJsonObject::Values keys are UE::FSharedString; copy per field.
	...
	for (TFieldIterator<FProperty> PropIt(StructDefinition); PropIt; ++PropIt)
	{
		...
		// Always pass direct value storage. This also keeps object elements inside
		// arrays/maps aligned with top-level object properties.
		void* ValuePtr = Property->ContainerPtrToValuePtr<void>(ContainerPtr, 0);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool FEGQuestJsonParser::JsonObjectStringToUStruct(const UStruct* StructDefinition, void* ContainerPtr)
{
	TSharedPtr<FJsonObject> JsonObject;
	TSharedRef<TJsonReader<>> JsonReader = TJsonReaderFactory<>::Create(JsonString);
	if (!FJsonSerializer::Deserialize(JsonReader, JsonObject) || !JsonObject.IsValid())
	{
		UE_LOG(LogQuestJsonParser, Error, TEXT("JsonObjectStringToUStruct - Unable to parse json=[%s]"), *JsonString);
		return false;
	}
	if (!JsonObjectToUStruct(JsonObject.ToSharedRef(), StructDefinition, ContainerPtr))
	{
		UE_LOG(LogQuestJsonParser, Error, TEXT("JsonObjectStringToUStruct - Unable to deserialize. json=[%s]"), *JsonString);
		return false;
	}
	return true;
}
