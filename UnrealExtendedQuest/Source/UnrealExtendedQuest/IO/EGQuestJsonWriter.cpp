...
DEFINE_LOG_CATEGORY(LogQuestJsonWriter);

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void FEGQuestJsonWriter::Write(const UStruct* StructDefinition, const void* ContainerPtr)
{
	EGQuestJsonWriterOptions WriterOptions;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
TSharedPtr<FJsonValue> FEGQuestJsonWriter::ConvertScalarPropertyToJsonValue(const FProperty* Property, const void* const ContainerPtr, const void* const ValuePtr)
{
	check(Property);
	if (bLogVerbose)
	{
		UE_LOG(LogQuestJsonWriter, Verbose, TEXT("ConvertScalarPropertyToJsonValue, Property = `%s`"), *Property->GetPathName());
	}
	if (ValuePtr == nullptr)
	{
		// Invalid
	...
	if (const auto* NameProperty = FNYReflectionHelper::CastProperty<FNameProperty>(Property))
	{
		...
		{
			UE_LOG(LogQuestJsonWriter, Error, TEXT("Got Property = `%s` of type FName but it is not valid :("), *NameProperty->GetNameCPP())
			return MakeShared<FJsonValueNull>();
		}
	...
	if (const auto* ArrayProperty = FNYReflectionHelper::CastProperty<FArrayProperty>(Property))
	{
		...
		const FEGQuestConstScriptArrayHelper Helper(ArrayProperty, ValuePtr);
	...
	if (const auto* MapProperty = FNYReflectionHelper::CastProperty<FMapProperty>(Property))
	{
		...
		const FEGQuestConstScriptMapHelper Helper(MapProperty, ValuePtr);
		...
		for (int32 Index = 0; Index < Helper.GetMaxIndex(); Index++)
		{
			...
			if (KeyElement.IsValid() && ValueElement.IsValid())
			{
				...
				if (KeyString.IsEmpty())
				{
					...
					if (KeyString.IsEmpty())
					{
						UE_LOG(LogQuestJsonWriter, Error, TEXT("Unable to convert key to string for property `%s`."), *MapProperty->GetNameCPP())
	...
	if (const auto* ObjectProperty = FNYReflectionHelper::CastProperty<FObjectProperty>(Property))
	{
		...
		// ValuePtr is direct property storage, including array/map elements.
		const UObject* ObjectPtr = ObjectProperty->GetObjectPropertyValue(ValuePtr);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
TSharedPtr<FJsonValue> FEGQuestJsonWriter::PropertyToJsonValue(const FProperty* Property, const void* const ContainerPtr, const void* const ValuePtr)
{
	check(Property);
	if (bLogVerbose)
	{
		UE_LOG(LogQuestJsonWriter, Verbose, TEXT("PropertyToJsonValue, Property = `%s`"), *Property->GetPathName());
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool FEGQuestJsonWriter::UStructToJsonAttributes(const UStruct* StructDefinition, const void* const ContainerPtr, TMap<FString, TSharedPtr<FJsonValue>>& OutJsonAttributes)
{
	...
	if (bLogVerbose)
	{
		UE_LOG(LogQuestJsonWriter, Verbose, TEXT("UStructToJsonAttributes, StructDefinition = `%s`"), *StructDefinition->GetPathName());
	}
	...
	for (TFieldIterator<const FProperty> It(StructDefinition); It; ++It)
	{
		...
		if (CheckFlags != 0 && !Property->HasAnyPropertyFlags(CheckFlags))
		{
			...
			if (bLogVerbose)
			{
				UE_LOG(LogQuestJsonWriter, Verbose, TEXT("Property = `%s` Does not have the required CheckFlags"), *Property->GetPathName());
			}
			continue;
		}
		if (CanSkipProperty(Property))
		{
			...
			if (bLogVerbose)
			{
				UE_LOG(LogQuestJsonWriter, Verbose, TEXT("Property = `%s` Marked as skiped"), *Property->GetPathName());
			}
			continue;
		}

		// Get the Pointer to the Value
		// Always pass direct value storage. FObjectProperty accessors handle the
		// pointer representation without applying the property offset twice.
		const void* ValuePtr = Property->ContainerPtrToValuePtr<void>(ContainerPtr, 0);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool FEGQuestJsonWriter::UStructToJsonString(const UStruct* StructDefinition, const void* const ContainerPtr,
	 const EGQuestJsonWriterOptions& Options, FString& OutJsonString)
{
	...
	UE_LOG(LogQuestJsonWriter, Error, TEXT("UStructToJsonObjectString - Unable to write out json"));
	return false;
}
