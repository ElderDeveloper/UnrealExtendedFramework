 ...
 * - limitation for each type you can see inside QuestIoTester.cpp in the Options.
 ...
 *
 *		- QuestNoExport: property is skipped - WARNING: should be only used on editor only variables,
 *			in non-editor builds you should use the properties from the SkipFlags
 *		- QuestWriteIndex: used on complex (struct/object) arrays, element index is exported as comment
 * 		- QuestLinePerItem: used to force primitive container to write each element into a new line (TODO: MAP SUPPORT)
 *		- QuestSaveOnlyReference: UObject path is serialized instead of UObject (can be used for DataAsset like objects stored in content browser)
 *			ATM IT ONLY WORKS IF IT IS NOT INSIDE A CONTAINER DIRECTLY (can be e.g. inside a struct inside a container tho)
 *
 */
class UNREALEXTENDEDQUEST_API IEGQuestWriter
{
public:
	virtual ~IEGQuestWriter() {}
...
#if WITH_EDITOR
		if (Property->HasMetaData(TEXT("QuestNoExport")))
		{
			return true;
		}
#endif
	...
	{
#if WITH_EDITOR
		return Property && Property->HasMetaData(TEXT("QuestLinePerItem"));
#else
		return false;
#endif
	}
	...
	{
#if WITH_EDITOR
		return Property && Property->HasMetaData(TEXT("QuestWriteIndex"));
#else
		return false;
#endif
	}
...
#if WITH_EDITOR
		return Property && Property->HasMetaData(TEXT("QuestSaveOnlyReference"));
#else
		return false;
#endif
	}
