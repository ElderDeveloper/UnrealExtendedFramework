...
// -- Presentation State (ModularSettings contract) --

/**
 * Cached per-player presentation prefs.
 * Seeded from project settings, then overwritten by ModularSettings Apply().
 */
USTRUCT(BlueprintType)
struct FEFSubtitlePresentationState
{
	GENERATED_BODY()
	...
	/** Optional style profile id (matches UEFSubtitleStyleProfile::ProfileId). */
	UPROPERTY(BlueprintReadWrite, Category = "Subtitle")
	FName StyleProfileId;
};
