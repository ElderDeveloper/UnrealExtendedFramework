...
#include "EFInputImageWidget.generated.h"
 ...
 */
UENUM(BlueprintType)
enum class EEFInputDeviceType : uint8
{
 ...
 */
UCLASS()
class UNREALEXTENDEDFRAMEWORK_API UEFInputImageWidget : public UUserWidget
{
	GENERATED_BODY()
...
public:
	...
	EEFInputDeviceType GetCurrentInputDevice() const { return CurrentInputDevice; }
...
protected:
	...
	UTexture2D* GetImageForDevice(EEFInputDeviceType DeviceType) const;
	...
	void UpdateDisplayedImage(EEFInputDeviceType DeviceType);
	
	void OnInputDetected(const FKey& Key);
	EEFInputDeviceType DetermineInputDeviceFromKey(const FKey& Key) const;

private:
	EEFInputDeviceType CurrentInputDevice = EEFInputDeviceType::KeyboardMouse;
	bool bIsListening = false;
	
	class FInputDeviceDetector : public IInputProcessor
	{
	public:
		FInputDeviceDetector(UEFInputImageWidget* InWidget)
			: Widget(InWidget)
		{
		}
	...
	private:
		TWeakObjectPtr<UEFInputImageWidget> Widget;
	};
