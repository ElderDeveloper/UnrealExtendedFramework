...
public:
	...
	/**
	 * Returns a random point uniformly distributed on the surface of a sphere centered at the origin.
	 * Equivalent to a unit direction * Radius (despite older "OutsideSphere" naming).
	 */
	UFUNCTION(BlueprintPure, Category="ExtendedFramework|Math|Random", meta=(DisplayName="Rand Point On Sphere", Keywords="OutsideSphere random surface"))
	static FVector RandPointOnSphere(float Radius);
