...
void FP2PNAT::SetRelaySettings()
{
	...
	Options.RelayControl = SampleConstants::RelaySetting;

	EOS_EResult Result = EOS_P2P_SetRelayControl(P2PHandle, &Options);
	if (Result == EOS_EResult::EOS_Success)
	{
		std::string RelaySettingString;

		switch (SampleConstants::RelaySetting)
		{
		case EOS_ERelayControl::EOS_RC_AllowRelays:
			RelaySettingString = "EOS_RC_AllowRelays";
			break;
		case EOS_ERelayControl::EOS_RC_ForceRelays:
			RelaySettingString = "EOS_RC_ForceRelays";
			break;
		case EOS_ERelayControl::EOS_RC_NoRelays:
			RelaySettingString = "EOS_RC_NoRelays";
			break;
		default:
			break;
		}
		FDebugLog::Log(L"EOS P2PNAT SetRelaySettings: SetRelaySettings [%ls]", FStringUtils::Widen(RelaySettingString).c_str());
	}
	else
	{
		FDebugLog::LogError(L"EOS P2PNAT SetRelaySettings: error while setting relay settings, code: %ls.", FStringUtils::Widen(EOS_EResult_ToString(Result)).c_str());
	}
}
...
void FP2PNAT::SendMessage(FProductUserId FriendId, const std::wstring& Message)
{
	...
	Options.Channel = 0;
	Options.Reliability = SampleConstants::PacketReliability;
	...
	if (Result == EOS_EResult::EOS_Success)
	{
		std::string PacketReliabilityString;
		switch (SampleConstants::PacketReliability)
		{
		case EOS_EPacketReliability::EOS_PR_ReliableOrdered:
			PacketReliabilityString = "EOS_PR_ReliableOrdered";
			break;
		case EOS_EPacketReliability::EOS_PR_ReliableUnordered:
			PacketReliabilityString = "EOS_PR_ReliableUnordered";
			break;
		case EOS_EPacketReliability::EOS_PR_UnreliableUnordered:
			PacketReliabilityString = "EOS_PR_UnreliableUnordered";
			break;
		default:
			break;
		}

		//Note: In a real game this log should be at a very verbose level
		FDebugLog::Log(L"EOS P2PNAT SendMessage: sent %ls packet to player[%ls]", FStringUtils::Widen(PacketReliabilityString).c_str(), FStringUtils::Widen(FAccountHelpers::ProductUserIDToString(FriendId)).c_str());
	}
	else
	{
...
void FP2PNAT::HandleReceivedMessages()
{
	...
	else if (Result == EOS_EResult::EOS_Success)
	{
		...
		if (P2PDialog)
		{
			...
			//Note: In a real game this log should be at a very verbose level
			FDebugLog::Log(L"EOS P2PNAT HandleReceivedMessages: received packet from player[%ls]", FStringUtils::Widen(FAccountHelpers::ProductUserIDToString(FriendId)).c_str());
		}
	}
	else
	{
