...
void FSession::InitFromSessionInfo(EOS_HSessionDetails SessionDetails, EOS_SessionDetails_Info* SessionInfo)
{
	if (SessionInfo && SessionInfo->Settings)
	{
		...
		OwnerUserId = SessionInfo->OwnerUserId;
	...
	// Try to fetch the session name from the attributes
	const auto NameAttribute = std::find_if(Attributes.cbegin(), Attributes.cend(), [](const Attribute& Attribute)
	{
		return Attribute.Key == SESSION_KEY_NAME;
	});
	if (NameAttribute != Attributes.cend() && NameAttribute->ValueType == Attribute::String)
	{
		Name = NameAttribute->AsString;
	}
...
bool FSessionMatchmaking::CreateSession(const FSession& Session)
{
	...
	for (const FSession::Attribute& NextAttribute : Session.Attributes)
	{
		...
		SetAttrResult = EOS_SessionModification_AddAttribute(ModificationHandle, &AttrOptions);
		if (SetAttrResult != EOS_EResult::EOS_Success)
		{
...
void FSessionMatchmaking::DestroyAllSessions()
{
	...
	const FProductUserId PlayerId = Player->GetProductUserID();
	std::vector<std::string> SessionsToDestroy = {};

	for (const std::pair<const std::string, FSession>& NextSession : CurrentSessions)
	{
		// We can't remove joined session
		if (NextSession.second.OwnerUserId == PlayerId)
		{
			// save the names of the sessions to destroy, as destroying them may modify the list we're iterating through
			SessionsToDestroy.push_back(NextSession.first);
		}
	}

	for (const std::string& SessionName : SessionsToDestroy)
	{
...
bool FSessionMatchmaking::HasActiveLocalSessions() const
{
	...
	return std::any_of(CurrentSessions.cbegin(), CurrentSessions.cend(),
		[PlayerId = Player->GetProductUserID()](const std::pair<const std::string, FSession>& NextSession)
	{
		return NextSession.second.OwnerUserId == PlayerId;
	});
}
...
void FSessionMatchmaking::Register(const std::string& SessionName, EOS_ProductUserId FriendId)
{
	...
	// Create a heap-allocated string so we can hand off the session name to the register callback
	// so we can refer to it, even if the provided reference is deallocated
	std::string* SessionNamePtr = new std::string(SessionName);

	EOS_Sessions_RegisterPlayers(SessionsHandle, &RegisterPlayersOptions, SessionNamePtr, OnRegisterCompleteCallback);
}
...
void FSessionMatchmaking::AcceptRequestToJoin(const FProductUserId& FriendProductUserId)
{
	...
	if (Player != nullptr)
	{
		...
		Register(PresenceSessionName, FriendProductUserId);
	}
	else
	{
		FDebugLog::LogError(L"Session Matchmaking - RequestToJoinSession: Current player is invalid!");
...
void FSessionMatchmaking::SubscribeToGameInvites()
{
	...
	EOS_CustomInvites_AddNotifyRequestToJoinResponseReceivedOptions RequestToJoinResponseReceivedOptions = {};
	RequestToJoinResponseReceivedOptions.ApiVersion = EOS_CUSTOMINVITES_ADDNOTIFYREQUESTTOJOINRESPONSERECEIVED_API_LATEST;
	RequestToJoinResponseReceivedNotificationHandle = EOS_CustomInvites_AddNotifyRequestToJoinResponseReceived(CustomInvitesHandle, &RequestToJoinResponseReceivedOptions, nullptr, OnRequestToJoinResponseReceivedCallback);
}
...
void FSessionMatchmaking::Search(const std::vector<FSession::Attribute>& Attributes)
{
	...
	if (Player != nullptr)
	{
		...
		// Only search for sessions that aren't full  (have room for 1+ users)
		ParamOptions.ComparisonOp = EOS_EOnlineComparisonOp::EOS_CO_GREATERTHANOREQUAL;
		AttrData.Key = EOS_SESSIONS_SEARCH_MINSLOTSAVAILABLE;
...
void FSessionMatchmaking::JoinSession(SessionDetailsKeeper SessionsDetail, bool bPresenceSession, bool bInvited)
{
	PlayerPtr Player = FPlayerManager::Get().GetPlayer(FPlayerManager::Get().GetCurrentUser());
	if (Player != nullptr)
	{
		// If we weren't invited to the session, let's ask the host if we can please join
		if (!bInvited)
		{
			EOS_SessionDetails_Info* SessionInfo = nullptr;
			EOS_SessionDetails_CopyInfoOptions CopyOptions = {};
			CopyOptions.ApiVersion = EOS_SESSIONDETAILS_COPYINFO_API_LATEST;
			EOS_EResult CopyResult = EOS_SessionDetails_CopyInfo(SessionsDetail.get(), &CopyOptions, &SessionInfo);
			if (CopyResult == EOS_EResult::EOS_Success)
			{
				// Ask to be invited
				RequestToJoinSession(SessionInfo->OwnerUserId);
				EOS_SessionDetails_Info_Release(SessionInfo);
				SetJoiningSessionDetails(SessionsDetail);
				return;
			}
		}

		const EOS_HSessions SessionsHandle = EOS_Platform_GetSessionsInterface(FPlatform::GetPlatformHandle());
		EOS_SessionDetails_CopySessionAttributeByKeyOptions CopySessionAttributeByKeyOptions = {};
		CopySessionAttributeByKeyOptions.ApiVersion = EOS_SESSIONDETAILS_COPYSESSIONATTRIBUTEBYKEY_API_LATEST;
		CopySessionAttributeByKeyOptions.AttrKey = SESSION_KEY_NAME;
		EOS_SessionDetails_Attribute* SessionAttribute = {};
		EOS_EResult CopySessionResult = EOS_SessionDetails_CopySessionAttributeByKey(SessionsDetail.get(), &CopySessionAttributeByKeyOptions, &SessionAttribute);
		std::string SessionName;
		if (CopySessionResult == EOS_EResult::EOS_Success)
		{
			const EOS_Sessions_AttributeData* const AttributeData = SessionAttribute->Data;
			if (AttributeData->ValueType == EOS_EAttributeType::EOS_AT_STRING)
			{
				SessionName = std::string(AttributeData->Value.AsUtf8);
			}

			EOS_SessionDetails_Attribute_Release(SessionAttribute);
		}
		...
		JoinOptions.SessionHandle = SessionsDetail.get();
		JoinOptions.SessionName = SessionName.c_str();
...
void FSessionMatchmaking::OnSearchResultsReceived()
{
	...
	if (JoinPresenceSessionId.size() > 0)
	{
		if (SessionDetailsKeeper Handle = CurrentSearch.GetSessionHandleById(JoinPresenceSessionId))
		{
			...
			JoinSession(Handle, bPresenceEnabled, true);
		}
		else
		{
			// Sample currently doesn't retry after any errors; inform the UI that the Join Game attempt is complete. 
...
void FSessionMatchmaking::OnJoinGameAcceptedByEventId(EOS_UI_EventId UiEventId)
{
	...
	{
		const bool bPresenceEnabled = true;
		JoinSession(EventSession, bPresenceEnabled, true);
	}
	else
	{
...
void EOS_CALL FSessionMatchmaking::OnRegisterCompleteCallback(const EOS_Sessions_RegisterPlayersCallbackInfo* Data)
{
	if (Data)
	{
		...
		else if (Data->RegisteredPlayersCount > 0)
		{
			// Fetch the local session instance
			const std::unique_ptr<FSessionMatchmaking>& Instance = FGame::Get().GetSessions();
			const FSession& ExistingSession = Instance->GetSession(*SessionNamePtr);
			if (ExistingSession != FSession::InvalidSession)
			{
				// If we are registering the local player & the local player is the owner,
				// don't use their registration to increase the number of connections as they are counted during creation
				uint32_t NumPlayersToAdd = Data->RegisteredPlayersCount;

				// Determine if the local player is the owner of the session
				const PlayerPtr Player = FPlayerManager::Get().GetPlayer(FPlayerManager::Get().GetCurrentUser());
				const bool IsLocalUserOwner = Player ? ExistingSession.OwnerUserId == Player->GetProductUserID() : false;

				// If the local player is the owner, check if we are trying to register them
				// If so, we don't need to and we can decrement the number of players to add.
				if (IsLocalUserOwner)
				{
					for (uint32_t Idx = 0; Idx < Data->RegisteredPlayersCount; ++Idx)
					{
						if (ExistingSession.OwnerUserId == Data->RegisteredPlayers[Idx])
						{
							// We are trying to register ourselves, the owner.
							// We don't need to include this in the number of players since it was done at creation.
							--NumPlayersToAdd;
							break;
						}
					}
				...
				{
					const auto FindSessionEntry = Instance->CurrentSessions.find(*SessionNamePtr);
					if (FindSessionEntry != Instance->CurrentSessions.end())
					{
						FindSessionEntry->second.NumConnections += NumPlayersToAdd;
					}
				}
			}
		}
...
void EOS_CALL FSessionMatchmaking::OnSendInviteCompleteCallback(const EOS_Sessions_SendInviteCallbackInfo* Data)
{
	if (Data)
	{
		...
		{
			FDebugLog::Log(L"Session Matchmaking (OnSendInviteCompleteCallback): invite to session sent successfully.");
		}
	}
	else
	{
...
void EOS_CALL FSessionMatchmaking::OnSessionInviteReceivedCallback(const EOS_Sessions_SessionInviteReceivedCallbackInfo* Data)
{
	if (Data)
	{
		...
		if (SessionDetailsKeeper SessionDetails = MakeSessionHandleByInviteId(Data->InviteId))
		{
			...
			{
				const std::unique_ptr<FSessionMatchmaking>& Instance = FGame::Get().GetSessions();
				Instance->SetInviteSession(InviteSession, SessionDetails);

				// Helper to fetch the session ID from the session details
				static auto GetSessionIdFromDetails = [](const SessionDetailsKeeper& Details)
				{
					...
					EOS_SessionDetails_CopyInfoOptions CopyOptions = {};
					CopyOptions.ApiVersion = EOS_SESSIONDETAILS_COPYINFO_API_LATEST;
					const EOS_EResult CopyResult = EOS_SessionDetails_CopyInfo(Details.get(), &CopyOptions, &SessionInfo);
					if (CopyResult == EOS_EResult::EOS_Success)
					{
						FoundSessionId.append(SessionInfo->SessionId);
						EOS_SessionDetails_Info_Release(SessionInfo);
					}
				...
				// Check if the invite we received is the one we're already trying to join
				// If so, no need to prompt - we can move forward with joining
				const std::string ReceivedInvite_SessionId = GetSessionIdFromDetails(SessionDetails);
				const std::string SeekingInvite_SessionId = GetSessionIdFromDetails(FGame::Get().GetSessions()->JoiningSessionDetails);
				if (!SeekingInvite_SessionId.empty() && SeekingInvite_SessionId == ReceivedInvite_SessionId)
				{
					const bool bPresenceEnabled = true;
					Instance->JoinSession(Instance->GetInviteSessionHandle(), bPresenceEnabled, true);
				}
				else
				{
					//Show popup
	...
	{
		FDebugLog::LogError(L"Session Matchmaking (OnSessionInviteReceivedCallback): Data is null");
	}
}
...
void EOS_CALL FSessionMatchmaking::OnSessionInviteAcceptedCallback(const EOS_Sessions_SessionInviteAcceptedCallbackInfo* Data)
{
	if (Data)
	{
		...
		{
			const bool bPresenceEnabled = true;
			FGame::Get().GetSessions()->JoinSession(InviteSessionKeeper, bPresenceEnabled, true);
		}
		else
		{
...
void EOS_CALL FSessionMatchmaking::OnSessionInviteRejectedCallback(const EOS_Sessions_SessionInviteRejectedCallbackInfo* Data)
{
	if (Data)
	{
		...
		//Hide popup
		FGameEvent Event(EGameEventType::OverlayInviteToSessionRejected);
		FGame::Get().OnGameEvent(Event);
	}
}
...
void EOS_CALL FSessionMatchmaking::OnSendRequestToJoinCompleteCallback(const EOS_CustomInvites_SendRequestToJoinCallbackInfo* Data)
{
	...
	{
		FDebugLog::Log(L"Session Matchmaking (OnSendRequestToJoinCompleteCallback): request to join session sent successfully.");
	}
}
...
void EOS_CALL FSessionMatchmaking::OnSendRequestToJoinAcceptedCallback(const EOS_CustomInvites_AcceptRequestToJoinCallbackInfo* Data)
{
	...
	{
		FDebugLog::Log(L"Session Matchmaking (OnSendRequestToJoinAcceptedCallback): accepting the other user's request to join the session confirmed successfully.");
	}
}
...
void EOS_CALL FSessionMatchmaking::OnRequestToJoinResponseReceivedCallback(const EOS_CustomInvites_RequestToJoinResponseReceivedCallbackInfo* Data)
{
	if (Data)
	{
		const std::wstring FromUserIdStr = FStringUtils::Widen(FAccountHelpers::ProductUserIDToString(Data->FromUserId));
		
		if (Data->Response == EOS_ERequestToJoinResponse::EOS_RTJR_ACCEPTED)
		{
			FDebugLog::Log(L"Session Matchmaking (OnRequestToJoinResponseReceivedCallback): User %s accepted the join request!", FromUserIdStr.c_str());

			const std::unique_ptr<FSessionMatchmaking>& Instance = FGame::Get().GetSessions();

			if (Instance->JoiningSessionDetails)
			{
				EOS_SessionDetails_Info* SessionInfo = nullptr;
				EOS_SessionDetails_CopyInfoOptions CopyOptions = {};
				CopyOptions.ApiVersion = EOS_SESSIONDETAILS_COPYINFO_API_LATEST;
				EOS_EResult CopyResult = EOS_SessionDetails_CopyInfo(Instance->JoiningSessionDetails.get(), &CopyOptions, &SessionInfo);

				if (CopyResult == EOS_EResult::EOS_Success)
				{
					auto& CurrentSessions = Instance->CurrentSessions;
					auto FoundSessionPairPtr = std::find_if(CurrentSessions.begin(), CurrentSessions.end(),
						[SessionId = SessionInfo->SessionId](const auto& SessionPair)
					{
						return SessionPair.second.Id == SessionId;
					});
					
					if (FoundSessionPairPtr != CurrentSessions.end())
					{
						// Found the session!
						FSession& FoundSession = FoundSessionPairPtr->second;
						Instance->Register(FoundSession.Name, Data->FromUserId);
						FoundSession.InitActiveSession();
					}
					else
					{
						// Couldn't find the session
						FDebugLog::Log(L"Session Matchmaking (OnRequestToJoinResponseReceivedCallback): Couldn't find a session matching the join request");
					}
				}
		...
		else if (Data->Response == EOS_ERequestToJoinResponse::EOS_RTJR_REJECTED)
		{
			FDebugLog::Log(L"Session Matchmaking: User %s rejected the join approval", FromUserIdStr.c_str());
		}
		else
		{
			FDebugLog::Log(L"Session Matchmaking: User %s provided an unknown response to the join approval", FromUserIdStr.c_str());
		}
	}
}
