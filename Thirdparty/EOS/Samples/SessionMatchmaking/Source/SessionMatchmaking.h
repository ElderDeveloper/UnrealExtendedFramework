...
const char* const SESSION_KEY_LEVEL = "LEVEL";
const char* const SESSION_KEY_NAME = "NAME";
...
public:
	...
	void JoinSession(SessionDetailsKeeper SessionsDetail, bool bPresenceSession, bool bInvited = false);
	...
	/**
	 * Callback that is fired when a response to a request to join is received
	 * @param Data Data received regarding the response
	 */
	static void EOS_CALL OnRequestToJoinResponseReceivedCallback(const EOS_CustomInvites_RequestToJoinResponseReceivedCallbackInfo* Data);
...
private:
	...
	EOS_NotificationId RequestToJoinResponseReceivedNotificationHandle = EOS_INVALID_NOTIFICATIONID;
