	...
	FDialog(InPos, InSize, InLayer)
{
	...
	AcceptInviteButton->SetOnPressedCallback([this]()
	{
		...
		if (Session.IsValid())
		{
			FGame::Get().GetSessions()->JoinSession(FGame::Get().GetSessions()->GetInviteSessionHandle(), PresenceSessionCheckbox->IsTicked(), true);
		}
		Hide();
	});
...
void FSessionInviteReceivedDialog::SetSessionInfo(const std::wstring& InFriendName, const FSession& InSession)
{
	...
	for (const FSession::Attribute& Attr : Session.Attributes)
	{
		if (Attr.Key == SESSION_KEY_LEVEL)
		{
			LevelName = FStringUtils::Widen(Attr.AsString);
		}
		else if (Attr.Key == SESSION_KEY_NAME)
		{
			SessionName = FStringUtils::Widen(Attr.AsString);
		}
	}
	...
	if (MessageLabel)
	{
		...
		{
			std::wstring MessageText = L"Session " + SessionName + L" Invite Received";
