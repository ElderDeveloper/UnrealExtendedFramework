...
void FLobbies::SendStringData(const std::string& InStringData)
{
	if (!CurrentLobby.IsValid())
	{
		FDebugLog::LogError(L"Lobbies - No active lobby!");
		return;
	}
	...
	const size_t CommandSize = sizeof(ELobbyRTCDataCommand) + InStringData.size();
	std::vector<uint8_t> Data(CommandSize, 0);

	Data[0] = static_cast<uint8_t>(ELobbyRTCDataCommand::StringData);
	memcpy(&Data[1], InStringData.c_str(), InStringData.size());

	EOS_RTCData_SendDataOptions SendDataOptions{};
	SendDataOptions.ApiVersion = EOS_RTCDATA_SENDDATA_API_LATEST;
	SendDataOptions.LocalUserId = CurrentUserProductId;
	SendDataOptions.RoomName = CurrentLobby.RTCRoomName.c_str();
	SendDataOptions.Data = Data.data();
	SendDataOptions.DataLengthBytes = static_cast<uint32_t>(CommandSize);
...
void FLobbies::OnStringDataReceived(FProductUserId ParticipantId, const std::wstring& StringData)
{
	// Find this participant in our list
	if (CurrentLobby.IsOwner(ParticipantId))
	{
		FGameEvent GameEvent(EGameEventType::LobbyStringDataReceived, StringData);
		FGame::Get().OnGameEvent(GameEvent);
	}
}
...
void FLobbies::OnRTCRoomDataReceived(const char* RoomName, FProductUserId ParticipantId, const void* Data, uint32_t DataLengthBytes)
{
	...
	const uint32_t PayloadLen = DataLengthBytes - static_cast<uint32_t>(sizeof(ELobbyRTCDataCommand));
	...
	case ELobbyRTCDataCommand::SkinColor:
		...
		break;
	case ELobbyRTCDataCommand::StringData:
		{
			if (PayloadLen > 0)
			{
				std::string StringData(static_cast<const char*>(Payload), PayloadLen);
				OnStringDataReceived(ParticipantId, FStringUtils::Widen(StringData));
			}
			else
			{
				FDebugLog::LogError(L"Lobbies (OnRTCRoomDataReceived): invalid string data.");
			}
		}
		break;
	default:
