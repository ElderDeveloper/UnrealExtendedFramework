...
#include "EFLoadingScreenManager.generated.h"

template <typename InterfaceType> class TScriptInterface;

class FSubsystemCollectionBase;
class IInputProcessor;
class IEFLoadingProcessInterface;
class SWidget;
class UObject;
class UWorld;
struct FFrame;
...
UCLASS()
class UNREALEXTENDEDFRAMEWORK_API UEFLoadingScreenManager : public UGameInstanceSubsystem, public FTickableGameObject
{
	GENERATED_BODY()
...
public:
	...
	void RegisterLoadingProcessor(TScriptInterface<IEFLoadingProcessInterface> Interface);
	void UnregisterLoadingProcessor(TScriptInterface<IEFLoadingProcessInterface> Interface);
...
private:
	...
	TArray<TWeakInterfacePtr<IEFLoadingProcessInterface>> ExternalLoadingProcessors;
	...
	uint32 LastKnownHighestAmountOfShaders = 0;
...
public:
	...
	float GetLoadingPercentage() const { return LoadingPercentage; }
