...
#include "EFLoadingScreenManager.h"
/*
...
#include "EFLoadingProcessInterface.h"
...
#include "Settings/EFLoadingScreenSettings.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(EFLoadingScreenManager)
...
//@TODO: Allow other things implementing IEFLoadingProcessInterface besides GameState/PlayerController (and owned components) to register as interested parties
...
bool IEFLoadingProcessInterface::ShouldShowLoadingScreen(UObject* TestObject, FString& OutReason)
{
	if (TestObject != nullptr)
	{
		if (IEFLoadingProcessInterface* LoadObserver = Cast<IEFLoadingProcessInterface>(TestObject))
		{
...
//////////////////////////////////////////////////////////////////////
// UEFLoadingScreenManager

void UEFLoadingScreenManager::Initialize(FSubsystemCollectionBase& Collection)
{
...
void UEFLoadingScreenManager::Deinitialize()
{
	StopBlockingInput();
	...
	// Does not exits on 5.4: SetTickableTickType(ETickableTickType::Never);
}


bool UEFLoadingScreenManager::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UEFLoadingScreenManager::Tick(float DeltaTime)
{
	UpdateLoadingScreen();
	...
	if (true)
	{
		uint32 RemainingShaders = FShaderPipelineCache::NumPrecompilesRemaining();
		if (LastKnownHighestAmountOfShaders < RemainingShaders)
		{
			LastKnownHighestAmountOfShaders = RemainingShaders;
		}
		if (RemainingShaders > 0)
		{
			LoadingPercentage = FMath::Clamp(static_cast<float>(LastKnownHighestAmountOfShaders - RemainingShaders) / static_cast<float>(LastKnownHighestAmountOfShaders), 0.0f, 1.0f);
		}
		else
		{
...
ETickableTickType UEFLoadingScreenManager::GetTickableTickType() const
{
	if (IsTemplate())
	{
...
bool UEFLoadingScreenManager::IsTickable() const
{
...
TStatId UEFLoadingScreenManager::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(UExtendedLoadingScreenManager, STATGROUP_Tickables);
}


UWorld* UEFLoadingScreenManager::GetTickableGameObjectWorld() const
{
	return GetGameInstance()->GetWorld();
}


void UEFLoadingScreenManager::RegisterLoadingProcessor(TScriptInterface<IEFLoadingProcessInterface> Interface)
{
	ExternalLoadingProcessors.Add(Interface.GetObject());
}


void UEFLoadingScreenManager::UnregisterLoadingProcessor(TScriptInterface<IEFLoadingProcessInterface> Interface)
{
	ExternalLoadingProcessors.Remove(Interface.GetObject());
}


void UEFLoadingScreenManager::HandlePreLoadMap(const FWorldContext& WorldContext, const FString& MapName)
{
...
void UEFLoadingScreenManager::HandlePostLoadMap(UWorld* World)
{
...
void UEFLoadingScreenManager::UpdateLoadingScreen()
{
...
bool UEFLoadingScreenManager::CheckForAnyNeedToShowLoadingScreen()
{
	...
	//out
	if (IEFLoadingProcessInterface::ShouldShowLoadingScreen(GameState,  DebugReasonForShowingOrHidingLoadingScreen))
	{
		return true;
	}
	...
	{
		if (IEFLoadingProcessInterface::ShouldShowLoadingScreen(TestComponent, DebugReasonForShowingOrHidingLoadingScreen))
		{
			return true;
		}
	}
	...
	// streaming in.
	//out
	for (const TWeakInterfacePtr<IEFLoadingProcessInterface>& Processor : ExternalLoadingProcessors)
	{
		if (IEFLoadingProcessInterface::ShouldShowLoadingScreen(Processor.GetObject(),DebugReasonForShowingOrHidingLoadingScreen))
		{
			return true;
		}
	}
	...
	for (ULocalPlayer* LP : LocalGameInstance->GetLocalPlayers())
	{
		if (LP != nullptr)
		{
			if (APlayerController* PC = LP->PlayerController)
			{
				...
				//out
				if (IEFLoadingProcessInterface::ShouldShowLoadingScreen(PC, DebugReasonForShowingOrHidingLoadingScreen))
				{
					return true;
				}
...
bool UEFLoadingScreenManager::ShouldShowLoadingScreen()
{
	const UEFLoadingScreenSettings* Settings = GetDefault<UEFLoadingScreenSettings>();
...
bool UEFLoadingScreenManager::IsShowingInitialLoadingScreen() const
{
...
void UEFLoadingScreenManager::ShowLoadingScreen()
{
...
void UEFLoadingScreenManager::HideLoadingScreen()
{
	if (!bCurrentlyShowingLoadingScreen)
	{
		return;
...
void UEFLoadingScreenManager::RemoveWidgetFromViewport()
{
	UGameInstance* LocalGameInstance = GetGameInstance();
...
void UEFLoadingScreenManager::StartBlockingInput()
{
...
void UEFLoadingScreenManager::StopBlockingInput()
{
	if (InputPreProcessor.IsValid())
	{
...
void UEFLoadingScreenManager::ChangePerformanceSettings(bool bEnabingLoadingScreen)
{
	UGameInstance* LocalGameInstance = GetGameInstance();
	UGameViewportClient* GameViewportClient = LocalGameInstance->GetGameViewportClient();
