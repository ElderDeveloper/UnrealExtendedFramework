...
#include "GAAsycTaskCooldownChanged.h"


UGAAsycTaskCooldownChanged * UGAAsycTaskCooldownChanged::GAListenForCooldownChange(UAbilitySystemComponent * AbilitySystemComponent, FGameplayTagContainer InCooldownTags, bool InUseServerCooldown)
{
	UGAAsycTaskCooldownChanged* ListenForCooldownChange = NewObject<UGAAsycTaskCooldownChanged>();
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(ListenForCooldownChange, &UGAAsycTaskCooldownChanged::OnActiveGameplayEffectAddedCallback);
	...
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag, EGameplayTagEventType::NewOrRemoved).AddUObject(ListenForCooldownChange, &UGAAsycTaskCooldownChanged::CooldownTagChanged);
	}

	return ListenForCooldownChange;
}

void UGAAsycTaskCooldownChanged::EndTask()
{
	if (IsValid(ASC))
	{
...
void UGAAsycTaskCooldownChanged::OnActiveGameplayEffectAddedCallback(UAbilitySystemComponent * Target, const FGameplayEffectSpec & SpecApplied, FActiveGameplayEffectHandle ActiveHandle)
{
...
void UGAAsycTaskCooldownChanged::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool UGAAsycTaskCooldownChanged::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float & TimeRemaining, float & CooldownDuration)
{
