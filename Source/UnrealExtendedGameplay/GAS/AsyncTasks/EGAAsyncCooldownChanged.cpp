...
#include "EGAAsyncCooldownChanged.h"



UEGAAsyncCooldownChanged * UEGAAsyncCooldownChanged::GAListenForCooldownChange(UAbilitySystemComponent * AbilitySystemComponent, FGameplayTagContainer InCooldownTags, bool InUseServerCooldown)
{
	UEGAAsyncCooldownChanged* ListenForCooldownChange = NewObject<UEGAAsyncCooldownChanged>();
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(ListenForCooldownChange, &UEGAAsyncCooldownChanged::OnActiveGameplayEffectAddedCallback);
	...
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag, EGameplayTagEventType::NewOrRemoved).AddUObject(ListenForCooldownChange, &UEGAAsyncCooldownChanged::CooldownTagChanged);
	}

	return ListenForCooldownChange;
}

void UEGAAsyncCooldownChanged::EndTask()
{
	if (IsValid(ASC))
	{
...
void UEGAAsyncCooldownChanged::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool UEGAAsyncCooldownChanged::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float & TimeRemaining, float & CooldownDuration)
{
